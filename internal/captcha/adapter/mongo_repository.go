package adapter

import (
	"context"
	"sync"
	"sync/atomic"
	"time"

	"github.com/samber/mo"
	"go.mongodb.org/mongo-driver/v2/bson"
	"go.mongodb.org/mongo-driver/v2/mongo"

	"todoe/internal/captcha/domain"
)

type StoredEvent struct {
	ID          bson.ObjectID `bson:"_id"`
	AggregateID bson.ObjectID `bson:"aggregate_id"`
	Type        string        `bson:"type"`
	Payload     bson.Raw      `bson:"payload"`
	CreatedAt   time.Time     `bson:"created_at"`
}

type MongoRepository struct {
	clientIO    mo.IOEither[*mongo.Client]
	once        sync.Once
	cached      mo.Either[error, *mongo.Client]
	initialized atomic.Bool
}

func NewMongoRepository(clientIO mo.IOEither[*mongo.Client]) *MongoRepository {
	return &MongoRepository{clientIO: clientIO}
}

func (r *MongoRepository) getClient() mo.Either[error, *mongo.Client] {
	r.once.Do(func() {
		r.cached = r.clientIO.Run()
		r.initialized.Store(true)
	})
	return r.cached
}

func (r *MongoRepository) db() (*mongo.Database, error) {
	either := r.getClient()
	if either.IsLeft() {
		return nil, either.MustLeft()
	}
	return either.MustRight().Database("todoe"), nil
}

func (r *MongoRepository) Append(ctx context.Context, aggregateID bson.ObjectID, eventType string, payload any) mo.Result[struct{}] {
	db, err := r.db()
	if err != nil {
		return mo.Err[struct{}](err)
	}
	raw, err := bson.Marshal(payload)
	if err != nil {
		return mo.Err[struct{}](err)
	}
	e := StoredEvent{
		ID:          bson.NewObjectID(),
		AggregateID: aggregateID,
		Type:        eventType,
		Payload:     raw,
		CreatedAt:   time.Now(),
	}
	if _, err := db.Collection("captcha_events").InsertOne(ctx, e); err != nil {
		return mo.Err[struct{}](err)
	}
	return mo.Ok(struct{}{})
}

func (r *MongoRepository) SaveChallenge(ctx context.Context, c domain.Challenge) mo.Result[struct{}] {
	db, err := r.db()
	if err != nil {
		return mo.Err[struct{}](err)
	}
	if _, err := db.Collection("captcha_challenges").InsertOne(ctx, c); err != nil {
		return mo.Err[struct{}](err)
	}
	return mo.Ok(struct{}{})
}

func (r *MongoRepository) FindChallenge(ctx context.Context, id bson.ObjectID) mo.Result[domain.Challenge] {
	db, err := r.db()
	if err != nil {
		return mo.Err[domain.Challenge](err)
	}
	var c domain.Challenge
	if err := db.Collection("captcha_challenges").FindOne(ctx, bson.D{{Key: "_id", Value: id}}).Decode(&c); err != nil {
		return mo.Err[domain.Challenge](err)
	}
	return mo.Ok(c)
}

func (r *MongoRepository) MarkVerified(ctx context.Context, id bson.ObjectID) mo.Result[struct{}] {
	db, err := r.db()
	if err != nil {
		return mo.Err[struct{}](err)
	}
	filter := bson.D{{Key: "_id", Value: id}}
	update := bson.D{{Key: "$set", Value: bson.D{{Key: "verified", Value: true}}}}
	if _, err := db.Collection("captcha_challenges").UpdateOne(ctx, filter, update); err != nil {
		return mo.Err[struct{}](err)
	}
	return mo.Ok(struct{}{})
}
