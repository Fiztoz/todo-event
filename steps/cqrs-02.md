# CQRS Step 2 — Implement GetTask in Service

## What changes and why

`GetTask` is a pure query — validate the ID is provided, delegate to the repository. No event publishing needed (queries are read-only).
...
// CreateTask, ListTasks, ChangeStatus exist — GetTask is missing
```
...
- `GetTask` is a query — no validation, no publishing, no side effects
- Delegates directly to `repo.FindByID`

## Next step

Add the `Detail` HTTP handler and error differentiation in `domain/task/adapter/http/handler.go`.
