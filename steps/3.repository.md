...
Create `domain/task/adapter/mongo_repository.go`.

Package `adapter`. Implements `port.Repository`. Same lazy-init pattern as `internal/health/adapter/mongo_repository.go`.
...
- Repository only touches MongoDB — no business logic
- Every method returns `mo.Result[T]`, never `(T, error)`
- Append-only: `Save` always inserts; no update or delete
- `FindAll` returns only leaf records — tasks with no children pointing to them via `origin_id`
...
Implement the application service in `domain/task/application/service.go`.
