...
The bus routes published events to registered handlers. Handler errors are logged but never surfaced — a failing side effect must not block the primary business flow.

## Target: `internal/event/bus.go`
...
- `Wildcard = "*"` — subscribe to receive every event type
- `HandlerFunc` adapts a plain function to the `Handler` interface
- `var _ Publisher = (*EventBus)(nil)` — compile-time check that `EventBus` satisfies `Publisher`
- Handlers run synchronously; errors are logged, never returned to the publisher

## Next step

Refactor the application service to publish events instead of calling `repo.Save` directly.
